...
@section('container')
    <div class="container">
        ...
        @if (auth()->user()->jabatan === 'Sekretaris')
            <a href="/surat-keluar/create" class="btn btn-info text-white m-3"><i class="bi bi-envelope-plus"></i> Tambah
                Data</a>
            <a href="/pengajuan/" class="btn btn-success text-white m-3"><i class="bi bi-eye"></i> Lihat Pengajuan</a>
        @endif
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if (auth()->user()->jabatan === 'Pengurus' || auth()->user()->jabatan === 'Ketua')
                                            <a href="/surat-keluar/{{ $surat->id }}" class="btn btn-success"><i
                                                    class="bi bi-eye"></i></a>
                                        @elseif (auth()->user()->jabatan === 'Pembina')
                                            <a href="/surat-keluar/{{ $surat->id }}" class="btn btn-success"><i
                                                    class="bi bi-eye"></i></a>
                                            <a href="/surat-keluar/{{ $surat->id }}/edit" class="btn btn-secondary"><i
                                                    class="bi bi-chat-dots"></i></a>
                                        @else
                                            <a href="/surat-keluar/{{ $surat->id }}" class="btn btn-success"><i
                                                    class="bi bi-eye"></i></a>
                                            <a href="/surat-keluar/{{ $surat->id }}/edit" class="btn btn-warning"><i
                                                    class="bi bi-pencil"></i></a>
                                            <form action="/surat-keluar/{{ $surat->id }}" method="post"
                                                class="d-inline">
                                                @method('delete')
                                                @csrf
