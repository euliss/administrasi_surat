...
@section('container')
        ...
        <div class="row bg-white mt-4 pt-3 p-1 rounded-3 shadow-sm">
            <form action="/surat-keluar" method="POST" enctype="multipart/form-data">
                ...
                @if (auth()->user()->jabatan === 'Sekretaris')
                    <input type="hidden" class="form-control" id="status" name="status" value="Tindaklanjuti" readonly>
                @endif
                <div class="mb-3">
                ...
                <div class="input-group mb-3">
                    ...
                    <input type="text" class="form-control @error('nomor_surat') is-invalid @enderror" id="nomor_surat"
                        name="nomor_surat" value="{{ old('nomor_surat') }}" required="required">
                    @error('nomor_surat')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
    ...
    <script>
        ...
        function pengkategorian() {
            if (kategori.value == 'surat undangan') {
                td1.style.display = 'none';
                console.log('hilang');
            } else {
                td1.style.display = 'initial';
                console.log('muncul');
